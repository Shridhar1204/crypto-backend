...
const Dashboard = () => {
  ...
  const fetchHoldings = async () => {
    try {
      ...
      const holdingResponse = await axios.get("http://localhost:8080/holdings/get", {
        headers: { Authorization: token },
      });
      setHoldings(holdingResponse.data.holdings);

      const statsResponse = await axios.get("http://localhost:8080/holdings/stats", {
        headers: { Authorization: token },
      });
      setStats({
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <section className="grid sm:grid-cols-3 gap-6">
          <div className="bg-gray-800 p-6 rounded-xl shadow text-center">
            ...
            <p className="text-3xl font-bold text-green-400">${stats.totalValue}</p>
          </div>
          <div className="bg-gray-800 p-6 rounded-xl shadow text-center">
            <p className="text-lg text-gray-400">Profit / Loss</p>
            <p className="text-3xl font-bold text-yellow-400">${stats.profitLoss}</p>
          </div>
          <div className="bg-gray-800 p-6 rounded-xl shadow text-center">
            <p className="text-lg text-gray-400">Total Holdings</p>
            <p className="text-3xl font-bold text-blue-400">{stats.totalHoldings}</p>
          </div>
        </section>
...
const AddHoldingForm = ({ onAdd }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-800 p-6 rounded-xl shadow space-y-4">
